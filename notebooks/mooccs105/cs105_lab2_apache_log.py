...
not200DF = logs_df.filter(~ logs_df['status'].rlike('200')) #
not200DF.show(10)
...
total_req_per_day_df = logs_df.select('host', dayofmonth(logs_df['time']).alias('day')).groupBy('day').count().withColumnRenamed('count', 'req')
...
avg_daily_req_per_host_df = temp_df.select(total_req_per_day_df['day'], (temp_df['req']/temp_df['count']).alias('avg_reqs_per_host_per_day'))
...
not_found_df = logs_df.filter(logs_df['status'].rlike('404'))
not_found_df.cache()
...
errors_by_date_sorted_df = not_found_df.select('host', dayofmonth(logs_df['time']).alias('day')).groupBy('day').count()
...
days_with_errors_404 = errors_by_date_sorted_df.map(lambda r: r[0]).collect()
errors_404_by_day = errors_by_date_sorted_df.map(lambda r: r[1]).collect()
...
hour_records_sorted_df = not_found_df.select('host', hour(logs_df['time']).alias('hour')).groupBy('hour').count()
...
hours_with_not_found = hour_records_sorted_df.map(lambda r: r[0]).collect()
not_found_counts_per_hour = hour_records_sorted_df.map(lambda r: r[1]).collect()
