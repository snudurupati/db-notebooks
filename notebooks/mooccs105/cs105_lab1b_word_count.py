...
pluralLengthsDF = pluralDF.select(length(pluralDF.word).alias("pluralLengths"))
...
def wordCount(wordListDF):
    ...
    """
    wordCountDF = wordListDF.groupBy("word").count()
    return wordCountDF
...
def removePunctuation(column):
    ...
    """
    return regexp_replace(trim(lower(column)), r'[^\w\s]|_', '').alias('column')
...
shakeWordsDF = (shakespeareDF
                .select(split(shakespeareDF.column, ' ').alias('word'))
